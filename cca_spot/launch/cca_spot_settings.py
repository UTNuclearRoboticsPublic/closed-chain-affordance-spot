...
def define_robot_paths_and_settings():
    ...
    Returns:
        dict: Configuration dictionary with the following keys:
            ...
            - sensors_3d_package: (Optional) Package containing 3D sensor config
            - sensors_3d_path: (Optional) Relative path to 3D sensor YAML
    """
    return {
        ...
        # ---- SENSORS 3D YAML(Modify if sensors available for environmental collision checking) ----
        "sensors_3d_package": "spot_moveit_config",
        "sensors_3d_rel_path": "config/sensors_3d.yaml",
...
def generate_sensors_3d_yaml_content(
    package_name: Optional[str],
    sensors_3d_yaml_rel_path: Optional[str]
) -> Optional[dict]:
    """Load 3D sensor configuration parameters for environmental collision checking with MoveIt.

    Reads a sensors_3d YAML file and returns it as a dict ready to be added
    to a node's parameters list. Returns None if no path is configured,
    allowing callers to skip it gracefully.

    Args:
        package_name: ROS 2 package containing the sensors YAML.
                      Pass None to disable sensor loading.
        sensors_3d_yaml_rel_path: Relative path to sensors YAML within the package.
                             Pass None to disable sensor loading.

    Returns:
        dict: Parsed YAML contents ready for use in node parameters, or
        None: If package_name or sensors_3d_yaml_rel_path is None (sensors disabled)

    Raises:
        RuntimeError: If the file is not found at the resolved path

    Example:
        >>> sensors_params = generate_sensors_3d_yaml_content("robot_moveit_config", "config/sensors_3d.yaml")
        >>> params_common = [
        ...     {"robot_description": robot_description},
        ...     *([ sensors_params ] if sensors_params else []),
        ... ]
    """
    if package_name is None or sensors_3d_yaml_rel_path is None:
        return None

    full_path = os.path.join(
        get_package_share_directory(package_name), sensors_3d_yaml_rel_path
    )
    ...
    try:
        ...
        raise RuntimeError(
            f"sensors_3d file not found: {full_path}\n"
            f"Ensure '{package_name}' package is installed and contains '{sensors_3d_yaml_rel_path}'"
        ) from e
